...
echo -n "Creating ${mysql_db}_recovered database... "
...
echo -n "Counting $mysql_db tables checksums... "
...
for t in $tables
do
	"$top_dir"/create_defs.pl --host $mysql_host --user $mysql_user --password "$mysql_password" --db $mysql_db --table $t > "$top_dir"/include/table_defs.h.$t
